...
<body>
    <div>
        <h1>
            Metre à jour le communiqué n {{ $num }}
        </h1>
        <form action="{{ route('update',$num) }}" method="put" enctype="multipart/form-data">
            @csrf
            <div>
                <label for="num">Communiqué no : </label><br>
                <input type="number" name="num" id="num" value="{{ old('num')? old('num') : $commun->num }}"> <br>
                @error('num')
                    <p style="color: red">
                        {{ $message }}
                    </p>
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title')? old('title') : $commun->title }}"> <br>
                @error('title')
                    <p style="color: red">
                        {{ $message }}
                    </p>
            ...
            <div>
                ...
                <input type="text" name="delai" id="delai" value="{{ old('delai') ? old('delai') : $commun->delai }}"> <br>
                @error('delai')
                    <p style="color: red">
                        {{ $message }}
                    </p>
            ...
            <div>
                ...
                <input type="file" name="file" id="file" value="{{ old('file')? old('file') : $commun->file }}">
            </div>
            @error('file')
                <p style="color: red">
                    {{ $message }}
